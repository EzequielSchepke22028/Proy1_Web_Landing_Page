from sqlalchemy import (
    ...
    DateTime, Text, ForeignKey, Enum, Numeric
)
...
class User(Base):
    ...
    reputation      = Column(Numeric(3, 2), default=0.0)
...
#La terminal del backend debería mostrar:
...
class User(Base):
    ...
    # Relaciones
    products        = relationship("Product",  back_populates="owner")
    orders_made     = relationship("Order", foreign_keys="Order.buyer_id",
                                   back_populates="buyer")
    orders_received = relationship("Order", foreign_keys="Order.seller_id",
...
#Guardá con `Ctrl+S`. El backend debería recargar solo en la terminal y mostrar:
#WARNING:  StatReload detected changes in 'app\models.py'. Reloading...
#INFO:     Application startup complete.
"""
"""from sqlalchemy import (
...
class Product(Base):
    ...
    # Numeric evita errores de punto flotante — importante para Data Science 📊
    price       = Column(Numeric(precision=12, scale=2), nullable=False)
    stock       = Column(Integer, default=0, nullable=False)

    # Categoría como string simple (más flexible que FK a tabla)
    category    = Column(String(100), nullable=True, index=True)
    image_url   = Column(String(500), nullable=True)
    is_active   = Column(Boolean, default=True, nullable=False)
